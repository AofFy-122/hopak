...
import PrintButton from '@/app/(protected)/billing/[id]/PrintButton'
...
export default async function TenantInvoiceDetailsPage(props) {
    ...
    const { data: invoice, error: invoiceError } = await supabase
        ...
        `)
        .eq('id', invoiceId)
        .eq('contracts.tenant_id', tenant.id) // Strict check
        .single()
    ...
    const tenantDetails = contract?.tenants
    ...
    const handlePayInvoice = async () => {
        ...
        redirect(`/tenant/billing/${invoice.id}`)
    }

    return (
        <div className="invoice-container">
            <div className="invoice-header">
                <Link href="/tenant/billing" className="btn btn-outline invoice-back-btn">
                    ← {t('back') || 'Back'}
                </Link>
                <div className="invoice-flex-spacer"></div>
                {invoice.status !== 'paid' && (
                    <form action={handlePayInvoice}>
                        <button type="submit" className="btn btn-primary">{t('markPaid') || 'Mark as Paid'}</button>
                    </form>
                )}
                <PrintButton />
            </div>
            ...
            <div className="card invoice-card">
                ...
                <div className="invoice-info-grid">
                    <div>
                        ...
                        <div className="invoice-tenant-name">{tenantDetails?.first_name} {tenantDetails?.last_name}</div>
                        <div className="invoice-tenant-details">
                            {t('room')}: {room?.room_number}<br />
                            {tenantDetails?.phone && <>{tenantDetails.phone}<br /></>}
                            {tenantDetails?.email}
                        </div>
                    </div>
                    <div>
                ...
                <table className="invoice-items-table">
                    ...
                    <tbody className="invoice-items-tbody">
                        ...
                        {invoice.invoice_items?.map((item, idx) => {
                            if (item.name === 'Room Rent') return null;
